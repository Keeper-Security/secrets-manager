...
- KSM-1140 - Added `allowSupplyHost` to the `PamSettings.connection` schema. Added audio control and browser session fields to `PamRemoteBrowserSettings.connection`: `disableAudio`, `disableCopy`, `disablePaste`, `audioChannels`, `audioBps`, `audioSampleRate`, `sessionPersistence`, `allowFileUploads`, `allowFileDownloads`, `ignoreInitialSslCert`, `recordingIncludeKeys`.
