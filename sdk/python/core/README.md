...
* KSM-1069 - Fixed the `error='key'` (server key-rotation) retry path looping indefinitely. Retries are now bounded at 3 attempts. When a custom server public key is configured (IL5 deployments), the SDK raises `KeeperError` immediately on the first key-rotation response instead of silently accepting a standard key, which would break the deployment.
