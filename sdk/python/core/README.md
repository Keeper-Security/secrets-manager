...
* KSM-1122 - Fixed the KSMCache cache file being created world-readable (mode 0644). The file contains the transmission key in cleartext. It is now written mode 0600 (owner read/write only). The SDK corrects existing files to 0600 on the next write.
...
* KSM-1152 - Fixed `RecordCreate.to_dict()` omitting `"custom"` from the serialized payload when no custom fields were set. The key is now always present as `[]`, to match the vault API expectation.
...
* KSM-1033 - Fixed throttle backoff jitter being two-sided (`[-25%, +25%]`), which could reduce the retry delay below its computed floor and trigger an immediate re-throttle. Jitter is now one-sided (`[0%, +25%]`). Also added a `MAX_THROTTLE_DELAY_SEC` cap (176s) on server-supplied `retry_after` values to prevent a misbehaving backend from forcing an arbitrarily long wait.
* KSM-1069 - Fixed the `error='key'` (server key-rotation) retry path looping indefinitely. Retries are now bounded at 3 attempts. When a custom server public key is configured (IL5 deployments), the SDK raises `KeeperError` immediately on the first key-rotation response. Accepting a standard key at that point would break the deployment.
