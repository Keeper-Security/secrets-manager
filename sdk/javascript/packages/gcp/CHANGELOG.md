...
- KSM-1534 - **The declared `engines.node` floor is now `>=22`, not `>=20`.** `@google-cloud/kms` 6.x (KSM-1510) and its own dependencies already require Node 22; declaring `>=20` here just delayed the same unsupported-runtime warning by one version. A Node 20 or 21 consumer that previously installed with only an `EBADENGINE` warning now sees this package's own `engines` field name the real floor. Node 20 reached end of life on 2026-04-30.
...
- KSM-1534 - Removed the `@tootallnate/once` override pinning it to `3.0.1`; it resolved nothing in the current dependency tree (`http-proxy-agent` is on `7.0.2`, which does not depend on `@tootallnate/once`), so the override was unnecessary regardless of its version. The `engines.node` floor change from this same ticket is listed under Changed above, since it drops support for a runtime.
