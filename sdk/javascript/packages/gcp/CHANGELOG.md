...
- KSM-1514 - **A config path that is a symbolic link is now refused on read too, not only replaced on write.** `init()` (via `loadConfig()`) and `decryptConfig()` both reject with an error naming the path, instead of silently reading through the link. Combined with the entry above, a symlinked `keyVaultConfigFileLocation` or `KSM_CONFIG_FILE` is no longer supported in either direction: point it at the real file.
...
- KSM-1514 - Closed a symlink-following read. An attacker able to create a file in the config directory, but not write the config file itself, could plant a symlink at the config path and have the SDK read an attacker-chosen `clientId` and `hostname`, redirecting every later request to an attacker-controlled endpoint. The refusal covers `loadConfig()`, `decryptConfig()`, and the existence check `createConfigFileIfMissing()` and `saveConfig()` share, including a dangling symlink at a not-yet-created config path, which the write-side atomic rename would otherwise silently replace with a real file, destroying the evidence a link was ever planted. The config directory is also now created at mode `0700` (was `0755`), so the precondition, an attacker able to write to the config directory at all, no longer holds for a freshly created directory. An already-existing directory is left at whatever mode it already had.
