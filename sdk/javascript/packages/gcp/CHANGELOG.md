...
- KSM-1533 - Raised the declared `axios` floor from `^1.13.5` to `^1.19.0`. The old range let a consumer who already pins an older axios keep an unpatched version through npm's dedupe, on the same client that carries a live GCP OAuth bearer token in `rawEncrypt`/`rawDecrypt`. Six advisories were fixed between 1.15.1 and 1.16.0 (`GHSA-6chq-wfr3-2hj9`, `GHSA-pf86-5x62-jrwf`, `GHSA-q8qp-cvcw-x6jj`, `GHSA-p92q-9vqr-4j8v`, `GHSA-35jp-ww65-95wh`, `GHSA-hfxv-24rg-xrqf`).
- KSM-1516 - Every public method except `init()` now rejects until `init()` has completed successfully, instead of running against an empty, never-loaded config. Before this fix, `isEmpty()` reported `true` against a fully populated config, and `saveString()`/`saveStorage()` resolved successfully while overwriting it, both on an instance that had never called `init()`. A caller that checks `isEmpty()` before deciding whether to bind a new device, the pattern the README's `oneTimeToken` note invites, could silently destroy the client ID, app key, and device private key of an already-bound device. The rejection is a typed `GCPKeyValueStorageError` naming `init()`, and it still applies if `init()` itself failed partway through: only a call that ran to completion marks the instance ready.
- KSM-1515 - `loadConfig()` now validates that the parsed config is a plain object of string values (a `Record<string, string>`) at both parse sites, the plaintext path and the decrypted path, and rejects anything else before either site acts on it. `JSON.parse()` succeeds for `null`, `0`, `false`, `""`, an array, and a primitive, none of which is the declared shape, and each previously took a different silently wrong path: falsy values left the file in plaintext while the log claimed encryption had started; an array made every later save a no-op that persisted nothing; a primitive got written to disk encrypted, and only the next `saveString()` threw a native `TypeError`. All three now reject immediately with a typed `GCPKeyValueStorageError`, before any write.
...
- KSM-1536 - `CHANGELOG.md` is now included in the published npm tarball (added to `files` in `package.json`). Added a Behavior Notes section to the README covering the v1.1.0 changes above. Added an explicit warning to the Decrypt Config section: `decryptConfig(true)` writes the client ID, app key, and device private key to disk in plaintext, and the file must be re-encrypted (by calling `init()` again) afterward. Documented the `KSM_CONFIG_FILE` environment variable and its precedence. Corrected the required IAM role list in both the README and `GcpKeyConfig.ts`'s doc comment: neither previously matched the code's actual GCP API calls. Removed `roles/cloudkms.admin`, which this package never needs since it never calls a key- or keyring-management operation, and added `roles/cloudkms.viewer`, which none of the other three listed roles include but which `getKeyDetails()` needs for `cloudkms.cryptoKeys.get`.
