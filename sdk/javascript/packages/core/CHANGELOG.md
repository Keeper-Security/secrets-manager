...
- KSM-1209 - Added a bounded, configurable request timeout to all network calls (main API requests, file upload, file download). Both platforms enforce it as a fixed deadline via `AbortController`, not Node's socket `timeout` option, which only resets on inactivity and can be held open indefinitely by a slow trickle of data. Node requests reject with a `KeeperError` when the deadline fires; browser requests fall back to a plain `setTimeout`-driven `AbortController` on runtimes that lack the `AbortSignal.timeout()` shorthand, instead of failing every request outright. Defaults to 30 seconds; override per call via `SecretManagerOptions.requestTimeoutMs`, or pass `timeoutMs` directly to `downloadFile`/`downloadThumbnail`, which now also fall back to `SecretManagerOptions.requestTimeoutMs` (as an optional 3rd argument) instead of only accepting an explicit override. An invalid `requestTimeoutMs`/`timeoutMs` (zero, negative, or non-finite) now throws immediately instead of silently disabling the timeout or firing almost instantly. Node's timeout error message no longer includes the request URL's query string, since file download/upload URLs from the storage backend carry a time-limited access token there. Previously a stalled or hostile server could hang the caller indefinitely.
