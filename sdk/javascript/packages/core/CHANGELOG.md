...
- KSM-1251 - Fixed throttle (HTTP 403) and server key-rotation detection running on an error body truncated to 1000 bytes. A throttle or key-rotation response padded past that size failed to parse, silently disabling the retry/rotation for that response. Both now run on the full body (decoding is capped at 64KB, well above any realistic body, to bound a pathological or malicious oversized response); the 1000-byte truncation still applies only to the message in the error the SDK throws when retries/rotation are exhausted or the response isn't recognized.
