...
- KSM-1255 - Fixed `options.serverPublicKeyId` and `options.serverPublicKey` being re-saved to storage on every call. For `serverPublicKeyId`, this undid a completed server key rotation and forced the client to re-rotate on every subsequent request; for `serverPublicKey`, it was an unnecessary write on every request. When only one field is supplied, it is now persisted once, on the first call against a fresh config, and left alone afterward; rotation (or a later call supplying both fields together, see below) owns any update to `serverPublicKeyId` after that. When both fields are supplied in the same call, they are now treated as one bound pair and rebound together whenever the incoming pair differs from what is stored, rather than as two independent write-once gates - this prevents a freshly-pinned key from ever being paired with a stale id left over from an earlier rotation or a partially-seeded config. A caller-supplied id outside the bundled key table (7-18) is now rejected immediately instead of being persisted unvalidated, and this check applies consistently whether or not a custom key is supplied in the same call: an id-only call now consults storage, not just the current call's options, to decide whether an already-pinned custom key makes an out-of-table id legitimate. The pinned custom-key path is unaffected: an out-of-table id alongside a custom key is still accepted.
