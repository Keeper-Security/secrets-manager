...
- KSM-1209 - Added a bounded, configurable request timeout to all network calls (main API requests, file upload, file download). Node requests now set a socket timeout and reject with a `KeeperError` if the server never responds; browser requests use `AbortSignal.timeout`. Defaults to 30 seconds; override per call via `SecretManagerOptions.requestTimeoutMs`, or pass `timeoutMs` directly to `downloadFile`/`downloadThumbnail`. Previously a stalled or hostile server could hang the caller indefinitely.
