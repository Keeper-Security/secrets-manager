...
- KSM-1251 - Fixed throttle (HTTP 403) and server key-rotation detection running on an error body truncated to 1000 bytes. A throttle or key-rotation response padded past that size failed to parse, silently disabling the retry/rotation for that response. Both now run on the full body (decoding is capped at 64KB, well above any realistic body, to bound a pathological or malicious oversized response); the 1000-byte truncation still applies only to the message in the error the SDK throws when the response isn't recognized (the throttle-exhaustion and key-rotation-exhaustion errors are synthetic messages with no body content). Also fixed the non-200 error path checking only truthiness of the response body: an empty body is `null` on Node but a truthy, zero-length array on browser, so a non-200 response with a genuinely empty body threw a blank error message on browser instead of the intended "unknown ksm error" fallback.
...
- KSM-1256 - Fixed `postQuery` permanently blocking every future request when the stored `serverPublicKeyId` fell outside the bundled key table (an id from a newer SDK build, or a corrupted value). `generateTransmissionKey` now falls back to the default bundled key in memory for that one request instead of throwing, and a later rotation hint or a fresh pin can still correct the stored id. A stored custom `serverPublicKey` with no paired `serverPublicKeyId` now fails loud at read time instead of silently reporting the wrong key id on the wire.
- KSM-1255 - Stopped `serverPublicKey`/`serverPublicKeyId` from being re-saved on every request when they already match what's stored, and closed a race where two concurrent requests sharing one `SecretsManager` instance could each write half of a new custom key/id pair. The pair is written key-before-id, validated together, and serialized per storage instance, including against a storage call that never settles, which no longer wedges every later request on that instance. A caller-supplied `serverPublicKeyId` is written once per pin rather than on every call, so replaying a stale options object after a server-driven key rotation no longer reverts it. A custom `serverPublicKey` now always requires its `serverPublicKeyId` in the same call, unless the call replays a key and id that already match storage. A stored `serverPublicKey` with no id, and no id supplied this call, still throws; supply both to recover. Two OS processes or browser tabs writing to one shared config file concurrently, and a rebind between two already-valid pairs failing mid-write, are not covered by this fix.
