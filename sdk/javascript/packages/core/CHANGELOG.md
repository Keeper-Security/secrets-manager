...
- KSM-1255 - Stopped `serverPublicKey`/`serverPublicKeyId` from being re-saved on every request when they already match what's stored, and closed a race where two concurrent requests sharing one `SecretsManager` instance could each write half of a new custom key/id pair, leaving a mismatched pair in storage - the two fields are now validated and updated together, serialized against concurrent calls. A custom `serverPublicKey` now always requires its `serverPublicKeyId` in the same call; supplying one without the other throws instead of silently pinning an incomplete or stale pair. Two OS processes or browser tabs writing to one shared config file concurrently is not covered by this fix.
