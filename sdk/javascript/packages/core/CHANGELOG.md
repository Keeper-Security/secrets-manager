...
- KSM-901 - Add IL5 (DoD Impact Level 5) support: region mapping (`IL5` → `il5.keepersecurity.us`) and custom server public key injection via three layers — the one-time token (4-segment `IL5:clientKey:keyId:serverPublicKey` form), the `serverPublicKey` / `serverPublicKeyId` config fields, and the `serverPublicKey` / `serverPublicKeyId` options on `SecretManagerOptions` (passed to `getSecrets`). When a custom key is configured, server-side key rotation hints are suppressed so the client stays pinned to the IL5 key.
