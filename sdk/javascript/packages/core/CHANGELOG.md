...
- KSM-1251 - Fixed throttle (HTTP 403) detection running on an error body truncated to 1000 bytes. A throttle response padded past that size failed to parse, silently disabling the retry added in 17.5.0 for that response. Throttle detection now runs on the full body; the 1000-byte truncation still applies to the message in the error the SDK throws when retries are exhausted or the response isn't a throttle.
