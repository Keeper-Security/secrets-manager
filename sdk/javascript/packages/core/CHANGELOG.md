...
- KSM-1255 - Fixed `options.serverPublicKeyId` and `options.serverPublicKey` being re-saved to storage on every call. For `serverPublicKeyId`, this undid a completed server key rotation and forced the client to re-rotate on every subsequent request; for `serverPublicKey`, it was an unnecessary write on every request. Both are now persisted once, on the first call against a fresh config; rotation owns every update to `serverPublicKeyId` after that. A caller-supplied id outside the bundled key table (7-18) is now rejected immediately instead of being persisted unvalidated. The pinned custom-key path is unaffected: an out-of-table id alongside a custom key is still accepted.
