...
mod proxy_integration_tests {
    ...
    fn test_proxy_with_qa_vault() {
        ...
        match secrets_manager.get_secrets(Vec::new()) {
            Ok(secrets) => {
                println!(
                    "✓ Successfully retrieved {} secrets through proxy!",
                    secrets.len()
                );
            ...
            Err(e) => {
                ...
                panic!(
                    "Proxy integration test failed. Common issues:\n\
    ...
    fn test_proxy_with_authentication() {
        ...
        let storage =
            InMemoryKeyValueStorage::new(Some(config_str)).expect("Failed to create storage");
        let config = KvStoreType::InMemory(storage);
        ...
        match secrets_manager.get_secrets(Vec::new()) {
            Ok(secrets) => {
                println!(
                    "✓ Authenticated proxy test passed! Retrieved {} secrets",
                    secrets.len()
                );
            }
            Err(e) => {
