...
class FullCRUDTest
  ...
  def test_create_complex_record
    ...
    begin
      ...
      # TOTP field
      complex_record.set_field('oneTimeCode',
      ...
      log 'Phone field verified', :success if phone && phone['number'] == '555-0123'

      name = created.get_field_value_single('name')
      log 'Name field verified', :success if name && name['first'] == 'Test'

      address = created.get_field_value_single('address')
      log 'Address field verified', :success if address && address['city'] == 'Test City'
  ...
  def test_notation_access
    ...
    begin
      ...
      notations.each do |notation|
        ...
        if value
          log "Retrieved via notation: #{notation.split('/').last} = #{value[0..20]}#{'...' if value.length > 20}",
              :success
        end
      rescue StandardError => e
