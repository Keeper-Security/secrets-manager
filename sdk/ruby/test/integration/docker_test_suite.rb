...
begin
  FileUtils.cp(File.join(sdk_dir, 'config.base64'), docker_dir)
...
puts "\nBy Test Suite:"
options[:tests].each do |test_file|
  results_for_test = results.map { |_v, t| t[test_file] }.compact
...
# Cleanup
FileUtils.rm_rf(docker_dir) unless options[:keep_containers]
