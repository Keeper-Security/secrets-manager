...
- **Fix**: KSM-1168 - `ksm sync --type aws` now requires `--prefix` when syncing records or folders (`--record`/`--folder`/`--folder-recursive`). The prefix is prepended to every AWS secret name derived from a record title, confining derived names to a namespace the operator controls. Dry-run output no longer includes the destination secret value; it reports whether the destination exists and whether it would change (`dstExists`/`dstDiffers`). Existing `--record`/`--folder` commands must add `--prefix`.
