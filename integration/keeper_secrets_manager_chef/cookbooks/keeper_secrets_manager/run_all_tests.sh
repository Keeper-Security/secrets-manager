...
if [ -z "$KEEPER_CONFIG" ]; then
    echo "⚠️ KEEPER_CONFIG environment variable not set!"
    echo "ℹ️ Please export your Keeper base64 config first:"
...
# 1. Python tests
echo "1️⃣ Running Python tests..."
./test_python.sh

# 2. Ruby syntax check
echo "2️⃣ Checking Ruby syntax..."
...
echo "3️⃣ Attempting ChefSpec tests..."
if command -v chef >/dev/null 2>&1; then
    if chef exec rspec --version >/dev/null 2>&1; then
        echo "Running ChefSpec with chef exec..."
        chef exec rspec || echo "⚠️ ChefSpec tests failed"
    else
        echo "⚠️ ChefSpec not available, skipping unit tests"
    fi
else
    echo "⚠️ Chef not available, skipping ChefSpec tests"
fi
...
EOF
    ...
    if [ "$USE_FALLBACK" = true ]; then
        echo "⚠️ Using fallback test config (will fail with real Keeper vault)"
    else
...
RUBY
        ...
        echo 'ℹ️ Using KEEPER_CONFIG environment variable for base64 authentication'
        ...
        if [ \"$USE_FALLBACK\" = \"true\" ]; then
            chef-client -z -c /tmp/client.rb -o keeper_secrets_manager::test_fetch --chef-license accept || echo '⚠️ Fetch recipe failed (expected - fallback config is not valid for real Keeper vault)'
        else
            echo '🎯 Running with your actual Keeper configuration...'
            chef-client -z -c /tmp/client.rb -o keeper_secrets_manager::test_fetch --chef-license accept || echo '⚠️ Fetch recipe failed - check your KEEPER_CONFIG and record UIDs'
        fi
        ...
        if [ -f /opt/keeper_secrets_manager/keeper_output.txt ]; then
            ...
            if [ \"$USE_FALLBACK\" = \"true\" ]; then
                echo '⚠️ No output file created (expected - fallback config cannot access real Keeper vault)'
            else
                echo '⚠️ No output file created - check your record UIDs in input.json'
            fi
        fi
        ...
        if [ -f /opt/keeper_secrets_manager/keeper_env.sh ]; then
            ...
            echo '⚠️ No environment file created'
        fi
        ...
        # Test help command
                  python3 scripts/ksm.py --help || echo '⚠️ Python script help failed'
        
        # Test with the input file
        echo '🧪 Testing with input file and your KEEPER_CONFIG...'
        if [ \"$USE_FALLBACK\" = \"true\" ]; then
            echo 'ℹ️ Expected to fail gracefully with fallback config'
            python3 scripts/ksm.py --input input.json || echo '⚠️ Expected failure - fallback config cannot access real Keeper vault'
        else
            echo 'ℹ️ Testing with your actual Keeper configuration'
            python3 scripts/ksm.py --input input.json || echo '⚠️ Failed - check your KEEPER_CONFIG and record UIDs'
        fi
        
        # Test authentication validation
        echo '🔍 Step 4: Testing base64 authentication flow...'
        echo 'ℹ️ The base64 authentication process:'
        ...
        echo 'ℹ️ Decoded base64 config structure:'
        echo \$KEEPER_CONFIG | base64 -d | python3 -m json.tool || echo '⚠️ Base64 decode test'
    ...
    echo "⚠️ Docker not available, skipping integration test"
fi

# 5. Code style check
echo "5️⃣ Checking code style..."
if command -v chef >/dev/null 2>&1; then
    chef exec cookstyle . || echo "⚠️ Cookstyle warnings found (non-blocking)"
else
    echo "⚠️ Chef not available, skipping style checks"
fi
