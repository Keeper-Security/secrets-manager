...
- JUnit test suite (23 tests) covering credential resolution logic, field-label diagnostics, and a PAM record regression.
- Compatibility matrix and FQCN registration guide in README.
- Yokohama (Patch 7+), Zurich, and Australia MID Server support.
- Actionable diagnostics for mislabeled Keeper fields, written to `agent.log` (informational only - the checks never throw or block a working credential): the resolver warns for every `mid_`-prefixed custom field whose suffix is not a recognized ServiceNow value name - catching the common mistake of using a ServiceNow form/column name (ex. `mid_private_key`, `mid_password`) instead of the interface value (`mid_privkey`, `mid_pswd`) - and when an unprefixed custom field's label exactly matches a supported value name. Every message lists the valid `mid_` labels (the `VAL_*` values from `IExternalCredential`, `snc-automation-api.jar`; a "did you mean ...?" hint is added for close typos) and is record-type aware. The supported value names are resolved at runtime from the `IExternalCredential` interface, with a built-in static list as fallback.
