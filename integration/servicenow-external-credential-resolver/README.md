...
* A `mid_`-prefixed custom field whose suffix is **not** one of the supported value names is **still passed through** (arbitrary `discovery_credential` columns are intentionally supported) but is flagged with a short warning; **once per lookup** the resolver also logs the **valid value names without the prefix** (the prefix is configurable, so it is stated once) - the `VAL_*` values from the `IExternalCredential` interface (`snc-automation-api.jar`). This catches the common mistake of copying a ServiceNow form/column name into the label (ex. `mid_private_key` or `mid_password`) instead of the interface value name (`mid_privkey`, `mid_pswd`) - so the field silently never mapped. (ServiceNow shows its own form/table names and maps them internally to these interface values.) A "did you mean ...?" suggestion is added when the suffix is a close typo (ex. `mid_authykey` → `mid_authkey`).
