...
/**
 * Keeper Secrets Manager external credential resolver for the ServiceNow MID Server.
 *
 * <p>ServiceNow loads this resolver by its fixed class name {@code com.snc.discovery.CredentialResolver}.
 * That name is shared by every MID Server external-credential-resolver JAR, so only one such resolver
 * JAR can be installed per MID Server - remove any other vendor's resolver JAR before deploying this
 * one. The pure, MID-free credential-resolution logic lives in {@link KeeperCredentialMapper} so it
 * can be unit tested without the MID Server on the classpath.</p>
 */
