...
const ksmService_1 = require("./services/ksmService");
const secretsProvider_1 = require("./providers/secretsProvider");
const notationProvider_1 = require("./providers/notationProvider");
const envSyncService_1 = require("./services/envSyncService");
const codeGenerationService_1 = require("./services/codeGenerationService");
const quickAccessService_1 = require("./services/quickAccessService");
const terminalDetectionService_1 = require("./services/terminalDetectionService");
const configurationManager_1 = require("./services/configurationManager");
const secretsTreeProvider_1 = require("./providers/secretsTreeProvider");
let ksmService;
let secretsProvider;
let notationProvider;
let envSyncService;
let codeGenerationService;
let quickAccessService;
let terminalDetectionService;
let configurationManager;
let secretsTreeProvider;
function activate(context) {
    console.log('Keeper Secrets Manager extension is now active!');
    // Initialize services
    configurationManager = new configurationManager_1.ConfigurationManager(context);
    ksmService = new ksmService_1.KSMService(context, configurationManager);
    secretsProvider = new secretsProvider_1.SecretsProvider(ksmService);
    notationProvider = new notationProvider_1.NotationProvider(ksmService);
    envSyncService = new envSyncService_1.EnvSyncService(ksmService);
    codeGenerationService = new codeGenerationService_1.CodeGenerationService(ksmService);
    quickAccessService = new quickAccessService_1.QuickAccessService(ksmService, context);
    terminalDetectionService = new terminalDetectionService_1.TerminalDetectionService(quickAccessService);
    secretsTreeProvider = new secretsTreeProvider_1.SecretsTreeProvider(configurationManager);
    // Initialize configuration manager
    configurationManager.initialize();
    // Register tree view
    console.log('Registering TreeView...');
    const treeView = vscode.window.createTreeView('keeper.secretsTree', {
        treeDataProvider: secretsTreeProvider,
        showCollapseAll: true,
        canSelectMany: false
    });
    console.log('TreeView registered:', treeView);
    ...
    const authenticateCommand = vscode.commands.registerCommand('keeper.tree.authenticate', async (configId) => {
        if (configId) {
    ...
    const refreshConfigCommand = vscode.commands.registerCommand('keeper.tree.refresh', async (configId) => {
        if (configId) {
    ...
    const removeConfigCommand = vscode.commands.registerCommand('keeper.tree.removeConfiguration', async (configId) => {
        if (configId) {
            await removeConfiguration(configId);
        }
    });
    const setActiveConfigCommand = vscode.commands.registerCommand('keeper.tree.setActiveConfiguration', async (configId) => {
        if (configId) {
    ...
    const copySecretCommand = vscode.commands.registerCommand('keeper.tree.copySecret', async (treeItem) => {
        ...
        if (treeItem && treeItem.configId && treeItem.recordId && treeItem.field) {
            ...
            console.error('Invalid tree item for copy:', treeItem);
        }
    });
    const treeGenerateCodeCommand = vscode.commands.registerCommand('keeper.tree.generateCode', async (treeItem) => {
        if (treeItem && treeItem.configId && treeItem.recordId && treeItem.field) {
            await codeGenerationService.showCodeGenerationPicker();
        }
    });
    const addToFavoritesCommand = vscode.commands.registerCommand('keeper.tree.addToFavorites', async (treeItem) => {
        if (treeItem && treeItem.configId && treeItem.recordId && treeItem.field) {
    ...
    const showExamplesCommand = vscode.commands.registerCommand('keeper.tree.showExamples', async (treeItem) => {
        if (treeItem && treeItem.configId && treeItem.recordId && treeItem.field) {
            await showSDKExamples(treeItem.recordId, treeItem.field.label || treeItem.field.type);
        }
    });
    const showRawDataCommand = vscode.commands.registerCommand('keeper.tree.showRawData', async (treeItem) => {
    ...
    const copyRecordUidCommand = vscode.commands.registerCommand('keeper.tree.copyRecordUid', async (treeItem) => {
    ...
    const downloadFileCommand = vscode.commands.registerCommand('keeper.tree.downloadFile', async (treeItem) => {
        if (treeItem && treeItem.configId && treeItem.recordId && treeItem.field) {
            await downloadFileFromField(treeItem.configId, treeItem.recordId, treeItem.field);
        }
    });
    const generateEditCodeCommand = vscode.commands.registerCommand('keeper.tree.generateEditCode', async (treeItem) => {
        if (treeItem && treeItem.record) {
            await showRecordEditExamples(treeItem.record.uid || treeItem.record.recordUid);
        }
    });
    const generateDeleteCodeCommand = vscode.commands.registerCommand('keeper.tree.generateDeleteCode', async (treeItem) => {
        if (treeItem && treeItem.record) {
            await showRecordDeleteExamples(treeItem.record.uid || treeItem.record.recordUid);
        }
    });
    const generateCreateCodeCommand = vscode.commands.registerCommand('keeper.tree.generateCreateCode', async (treeItem) => {
        if (treeItem && treeItem.record) {
    ...
    const insertSecretCommand = vscode.commands.registerCommand('keeper.insertSecret', async (treeItem) => {
    ...
    const addToEnvCommand = vscode.commands.registerCommand('keeper.addToEnv', async (treeItem) => {
        if (treeItem && treeItem.record && treeItem.configId) {
            // Pre-select the record from tree context
    ...
    // Register providers
    const completionProvider = vscode.languages.registerCompletionItemProvider({ scheme: 'file' }, notationProvider, '$', '{', ':', '/');
    const hoverProvider = vscode.languages.registerHoverProvider({ scheme: 'file' }, notationProvider);
    context.subscriptions.push(treeView, addConfigurationCommand, authenticateCommand, refreshConfigCommand, removeConfigCommand, setActiveConfigCommand, copySecretCommand, treeGenerateCodeCommand, addToFavoritesCommand, showExamplesCommand, showRawDataCommand, copyRecordUidCommand, downloadFileCommand, generateEditCodeCommand, generateDeleteCodeCommand, generateCreateCodeCommand, legacyAuthenticateCommand, listSecretsCommand, refreshSecretsCommand, insertSecretCommand, logoutCommand, syncEnvCommand, addToEnvCommand, generateEnvTemplateCommand, generateCodeCommand, saveSecretCommand, quickAccessCommand, completionProvider, hoverProvider, quickAccessService, terminalDetectionService);
...
async function authenticateConfiguration(configId) {
    try {
...
async function refreshConfiguration(configId) {
    try {
...
async function removeConfiguration(configId) {
    const config = configurationManager.getConfiguration(configId);
    if (!config) {
        return;
    }
    const confirm = await vscode.window.showWarningMessage(`Are you sure you want to remove the configuration "${config.displayName}"?`, { modal: true }, 'Remove', 'Cancel');
    if (confirm === 'Remove') {
        try {
...
async function copySecretFromTree(configId, recordId, fieldType) {
    try {
        const config = configurationManager.getConfiguration(configId);
        if (!config || !config.records) {
        ...
        const field = record.data.fields.find((f) => f.type === fieldType || f.label === fieldType);
        if (!field) {
        ...
        if (config_settings.get('clipboardAutoClear', true)) {
            setTimeout(async () => {
...
async function syncEnvironmentSecrets() {
    ...
    try {
        ...
        const selectedFile = await vscode.window.showQuickPick(existingFiles.map(f => ({ label: f.name, description: f.path, file: f })), { placeHolder: 'Select .env file to sync' });
        if (!selectedFile)
            return;
        // Ask for dry run
        const dryRun = await vscode.window.showQuickPick(['Yes, sync now', 'No, show preview first (dry run)'], { placeHolder: 'Do you want to sync immediately or preview changes first?' });
        if (!dryRun)
            return;
        ...
        let backup = false;
        if (!isDryRun) {
            const backupChoice = await vscode.window.showQuickPick(['Yes, create backup', 'No, sync without backup'], { placeHolder: 'Create backup before syncing?' });
        ...
        if (isDryRun) {
            ...
            vscode.window.showInformationMessage(`Synced ${result.updated.length} variables${result.errors.length > 0 ? ` (${result.errors.length} errors)` : ''}`);
        }
...
async function addSecretToEnv() {
    ...
    try {
        ...
        const selectedFile = await vscode.window.showQuickPick(envFiles.map(f => ({ label: f.name, description: f.exists ? 'exists' : 'will be created', file: f })), { placeHolder: 'Select .env file to add secret to' });
        if (!selectedFile)
            return;
        // Get variable name
        ...
        const selectedSecret = await vscode.window.showQuickPick(secrets.map(s => ({ label: s.data.title || s.recordUid, description: s.recordUid, secret: s })), { placeHolder: 'Select secret' });
        if (!selectedSecret)
            return;
        // Let user select field
        const selectedField = await vscode.window.showQuickPick(selectedSecret.secret.data.fields.map((f) => ({
        ...
        if (templateValue === undefined)
            return;
        await envSyncService.addSecretToEnvFile(selectedFile.file.path, varName, selectedSecret.secret.recordUid, selectedField.field.label || selectedField.field.type, templateValue);
    }
    catch (error) {
...
async function generateEnvTemplate() {
    ...
    try {
        ...
        const selectedFile = await vscode.window.showQuickPick(envFiles.map(f => ({ label: f.name, description: f.exists ? 'will be overwritten' : 'will be created', file: f })), { placeHolder: 'Select .env file to generate template for' });
        if (!selectedFile)
            return;
...
async function saveSecretToVault(args) {
    const activeConfig = configurationManager.getActiveConfiguration();
    if (!activeConfig || !activeConfig.isAuthenticated) {
        vscode.window.showWarningMessage('Please authenticate with a configuration first');
        return;
    ...
    try {
        ...
        const confirm = await vscode.window.showWarningMessage(`Do you want to save this ${data.type} to Keeper Vault?`, { modal: true }, 'Yes, Save', 'Cancel');
        if (confirm !== 'Yes, Save') {
            return;
        }
        ...
        const replaceChoice = await vscode.window.showInformationMessage('Secret saved successfully! Do you want to replace the hardcoded value with an environment variable?', 'Yes, Replace', 'No, Keep As Is');
        if (replaceChoice === 'Yes, Replace') {
            ...
            if (varName) {
                ...
                const range = new vscode.Range(data.range.start.line, data.range.start.character, data.range.end.line, data.range.end.character);
                edit.replace(document.uri, range, `process.env.${varName}`);
                await vscode.workspace.applyEdit(edit);
                // Add to .env file
                await envSyncService.addSecretToEnvFile('.env', varName, recordUid, 'password', `keeper://${recordUid}/field/password`);
                vscode.window.showInformationMessage(`Secret saved and replaced with ${varName}! Added to .env file with keeper reference.`);
            }
        }
        else {
...
async function showSDKExamples(recordId, fieldName) {
...
async function showRawRecordData(configId, recordId) {
    try {
        const config = configurationManager.getConfiguration(configId);
        if (!config || !config.records) {
            throw new Error('Configuration not found or not authenticated');
...
async function addSecretToEnvFromTree(configId, record) {
    try {
        const config = configurationManager.getConfiguration(configId);
        if (!config || !config.records) {
            vscode.window.showErrorMessage('Configuration not found or not authenticated');
        ...
        if (!varName)
            return;
        // Let user select field
        const selectedField = await vscode.window.showQuickPick(record.data.fields.map((f) => ({
            label: f.label || f.type,
            description: f.type,
            field: f
        })), { placeHolder: 'Select field' });
        ...
        if (templateValue === undefined)
            return;
        await envSyncService.addSecretToEnvFile(selectedFile.file.path, varName, record.recordUid, selectedField.field.label || selectedField.field.type, templateValue);
...
async function insertSecretReferenceFromTree(configId, record) {
    try {
        const config = configurationManager.getConfiguration(configId);
        if (!config || !config.records) {
            vscode.window.showErrorMessage('Configuration not found or not authenticated');
...
async function downloadFileFromField(configId, recordId, field) {
    try {
        const config = configurationManager.getConfiguration(configId);
        if (!config || !config.records) {
            throw new Error('Configuration not found or not authenticated');
...
async function selectLanguageForCodeGeneration() {
    const languages = [
...
function generateJavaScriptEditExample(recordUid) {
    return `
## JavaScript/Node.js
\`\`\`javascript
...
function generatePythonEditExample(recordUid) {
    return `
## Python
\`\`\`python
from keeper_secrets_manager_core import SecretsManager
...
function generateGoEditExample(recordUid) {
    return `
## Go
\`\`\`go
package main
...
function generateJavaEditExample(recordUid) {
    return `
## Java
\`\`\`java
import com.keepersecurity.secretsManager.core.SecretsManager;
...
function generateCSharpEditExample(recordUid) {
    return `
## .NET/C#
\`\`\`csharp
using SecretsManager;
...
function generateCliEditExample(recordUid) {
    return `
## CLI/Bash
\`\`\`bash
...
async function showRecordEditExamples(recordUid) {
...
async function showRecordDeleteExamples(recordUid) {
    const language = await selectLanguageForCodeGeneration();
    if (!language)
        return;
...
async function showRecordCreateExamples() {
    const language = await selectLanguageForCodeGeneration();
    if (!language)
        return;
