...
public async Task GetAllSecrets(SecretsManagerOptions options)
{
    ...
    getTemplates(language, category) {
    ...
    async generateCode(template, values) {
    ...
    async showCodeGenerationPicker() {
        ...
        const selectedCategory = await vscode.window.showQuickPick(categories.map(cat => ({
        ...
        const selectedTemplate = await vscode.window.showQuickPick(templates.map(template => ({
            label: template.name,
    ...
    async selectSecret(placeholder) {
        ...
        const selectedSecret = await vscode.window.showQuickPick(secrets.map(s => ({
    ...
    getLanguageId(language) {
        ...
        return languageIds[language] || 'plaintext';
    }
}
