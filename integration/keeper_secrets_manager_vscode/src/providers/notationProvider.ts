...
export class NotationProvider implements vscode.CompletionItemProvider, vscode.HoverProvider {
    ...
    private provideUidCompletions(): vscode.CompletionItem[] {
    ...
    private provideFieldCompletions(uid: string): vscode.CompletionItem[] {
        ...
        return record.data.fields.map((field: any) => {
    ...
    private detectPotentialSecret(document: vscode.TextDocument, position: vscode.Position): {
        type: string;
        value: string;
        maskedValue: string;
        range: vscode.Range;
            ...
            while ((match = pattern.regex.exec(text)) !== null) {
                ...
                if (char >= matchStart && char <= matchEnd) {
                    ...
                    const range = new vscode.Range(
                        new vscode.Position(line.lineNumber, matchStart),
                        new vscode.Position(line.lineNumber, matchEnd)
                    );
